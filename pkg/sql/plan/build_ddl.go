...
func buildIndexTable(createTable *plan.CreateTable, uniqueIndexInfos []*tree.UniqueIndex, indexInfos []*tree.Index, colMap map[string]*ColDef, pkeyName string) error {
	...
	for _, indexInfo := range uniqueIndexInfos {
		...
		if len(indexInfo.KeyParts) == 1 {
			...
			keyName = util.BuildCompositePrimaryKeyColumnName(field.Names)
			if keyName == pkeyName {
				continue
			}
			if _, ok := uniqueNameCheck[keyName]; ok {
	...
	// now index can just accelerate primary key
	// if the table don't have primary key, we don't create index table
...
func buildDropTable(stmt *tree.DropTable, ctx CompilerContext) (*Plan, error) {
	...
	if tableDef == nil {
		...
		indexTableInfos := BuildIndexTableInfos(ctx, dropTable.Database, tableDef.Defs)
		dropTable.IndexTableNames = make([]string, len(indexTableInfos))
		for i := range indexTableInfos {
			dropTable.IndexTableNames[i] = indexTableInfos[i].TableName
		}
	}
	return &Plan{
		Plan: &plan.Plan_Ddl{
